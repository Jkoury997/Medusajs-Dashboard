...
function renderSectionHtml(section: ContentSection, index: number): string {
  switch (section.type) {
    case "text":
...
export function EmailPreview({ heading, sections, buttonText, buttonUrl, footerText }: EmailPreviewProps) {
  const html = useMemo(() => {
    const sectionsHtml = sections.map((s, i) => renderSectionHtml(s, i)).join("\n")
...
<body>
  <div class="container">
    ...
    <div class="footer">
      <p>${footerText || "Si tenes alguna pregunta, ponete en contacto con nuestro equipo en"} <a href="mailto:info@marcelakoury.com">info@marcelakoury.com</a></p>
